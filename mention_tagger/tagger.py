...
bert_dim=768
modelSize=4*bert_dim


class Tagger(BertPreTrainedModel):

	def __init__(self, config, freeze_bert=False, tagset=None, tagset_flat=None, hidden_dim=100, flat_hidden_dim=100, device=None):
	...
	def forwardFlatSequence(self, input_ids, token_type_ids=None, attention_mask=None, transforms=None, labels=None):
		...
		""" Forward training pass for flat sequence labeling """
	...
	def forward(self, input_ids, matrix1, matrix2, attention_mask=None, transforms=None, labels=None, lens=None):
		...
		""" Forward training pass for layered sequence labeling """
		...
		with torch.no_grad():
			...
			for b in range(len(batched_data)):
				...
				for d in range(len(all_tags1)):
					...
					for entity in self.get_spans(c, all_tags1[d], batched_orig_token_lens[b][d], batched_sents[b][d][1:]):
						preds[entity]=1
					for entity in self.get_spans(c, all_tags2[d], batched_orig_token_lens[b][d], batched_sents[b][d][1:]):
						preds[entity]=1
					for entity in self.get_spans(c, all_tags3[d], batched_orig_token_lens[b][d], batched_sents[b][d][1:]):
						preds[entity]=1
		...
		with torch.no_grad():
			...
			for b in range(len(test_batched_data)):
				...
				for d in range(len(all_tags1)):
					...
					for entity in self.get_spans(c, test_batched_layered_labels1[b][d][1:], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						golds[entity]=1
					for entity in self.get_spans(c, test_batched_layered_labels2[b][d][1:], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						golds[entity]=1
					for entity in self.get_spans(c, test_batched_layered_labels3[b][d][1:], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						golds[entity]=1
					for entity in self.get_spans(c, test_batched_layered_labels4[b][d][1:], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						golds[entity]=1
					...
					for entity in self.get_spans(c, all_tags1[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						preds[entity]=1
					for entity in self.get_spans(c, all_tags2[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						preds[entity]=1
					for entity in self.get_spans(c, all_tags3[d], dev_lens[0][b][d], test_batched_sents[b][d][1:]):
						preds[entity]=1

					c+=1

			# F1=sequence_eval.check_span_f1_two_dicts(golds, preds)
			F1=sequence_eval.check_span_f1_just_spans(golds, preds)
	...
	def evaluateFlat(self, dev_batched_data, dev_batched_mask, dev_batched_labels, dev_batched_transforms, metric, tagset):
		...
		""" Evaluate input data (with labels) for flat sequence labeling """
	...
	def tagFlat(self, batched_sents, batched_data, batched_mask, batched_transforms, batched_orig_token_lens, ordering):
		...
		""" Tag input data for flat sequence labeling """
	...
	def get_spans(self, doc_idx, tags, length, sentence):
		
		""" Get all labeled spans from predictions """
		...
		for idx, tag in enumerate(tags):
			...
			if tag.startswith("B-"):
				...
				while(1):
					...
					tagn=self.rev_tagset[int(tags[j])]
	...
	def compress(self, labels):
		...
		for i in range(len(labels)):
			...
			newkeep=[]
			for j in keep:
				if labels[i][j] == -100 or not self.rev_tagset[labels[i][j]].startswith("I-"):
					newkeep.append(j)
	...
	def get_index(self, all_labels):
		...
		for labels in all_labels:
			...
			for idx, label in enumerate(labels):
				...
				if label == -100 or not self.rev_tagset[label].startswith("I-"):
					ind[idx]=1		
					index.append(ind)
				else:
					index[-1][idx]=1
