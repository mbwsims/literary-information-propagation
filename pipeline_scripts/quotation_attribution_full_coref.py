			...
			if tid in children:
				for child in children[tid]:
					if tokens[child][13] == "O" and tokens[child][12]== "nsubj":
						...
						# sent=[]
						# for i in range(tid-10, tid+10):
						# 	sent.append(tokens[i][8])
						# print(' '.join(sent))
						if len(cands) > 0:
						# 	print("----------WINNER", tokens[cands[0][0]][8])
							return cands[0]
...
# 	ap.add_argument("--conllFolder", required=True)
